...
const DEFAULTS = {
  ...
  // A STALL IS NOT THE LINK. Once there is a fit, a sample slower than
  // stallFactor x its prediction + stallSlackMs is set aside rather than fitted:
  // least squares has no defence against one, and a single held transfer --
  // 1.5 s against a predicted 30 ms -- drove the intercept to 250 ms and the
  // slope negative, so the fit was refused and the defaults came back. If
  // resetAfter samples IN A ROW are set aside, the link has changed rather
  // than stalled: the window is cleared and they are taken as the new truth.
  stallFactor: 4,
  stallSlackMs: 20,
  resetAfter: 4,
};
...
export function createEstimator(opts = {}) {
  ...
  const xs = [], ys = [];
  let fit = null, rejected = 0, streak = [];
  ...
  return {
    ...
    // Returns what became of it: "fitted", "stall" (set aside) or "bad".
    observe(bytes, ms) {
      if (!(bytes > 0) || !(ms >= 0)) return "bad";
      if (fit && ms > P.stallFactor * this.predictMs(bytes) + P.stallSlackMs) {
        rejected++;
        streak.push([bytes, ms]);
        if (streak.length < P.resetAfter) return "stall";
        xs.length = 0; ys.length = 0;      // not a stall: the link changed
        for (const [b, m] of streak) { xs.push(b); ys.push(m); }
        streak = [];
        refit();
        return "fitted";
      }
    ...
    reset() { xs.length = 0; ys.length = 0; fit = null; streak = []; },
  };
}
