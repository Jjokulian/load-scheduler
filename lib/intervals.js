...
//
// A GAP THAT OVERLAPS `avoid` IS NEVER BRIDGED. When the runs come from a
// subtraction, the holes between them are often exactly the bytes already
// held or already in flight -- and bridging one fetches those bytes again.
// Subtracting first does not prevent that; only refusing the bridge does.
//
// ONE POINTER, NOT ONE SUBTRACTION PER HOLE. Holes arrive in address order,
// so the first blocked range that could touch the next hole never moves
// backwards. Testing each hole against the whole blocked list with subtract()
// was O(holes x blocked): a whole-store plan with 1,851 held holes took
// 1,716 ms against 5 ms without them. This is O(holes + blocked), and a test
// checks it against the old per-hole form on random inputs.
export function bridge(list, gap, avoid = []) {
  const xs = normalise(list);
  if (xs.length < 2 || !(gap > 0)) return xs;
  const blocked = normalise(avoid);
  let j = 0;
  const out = [xs[0]];
  for (let i = 1; i < xs.length; i++) {
    const last = out[out.length - 1];
    const lo = last.hi, hi = xs[i].lo;
    while (j < blocked.length && blocked[j].hi <= lo) j++;
    const clear = !(j < blocked.length && blocked[j].lo < hi);
    if (hi - lo <= gap && clear) last.hi = Math.max(last.hi, xs[i].hi);
    else out.push({ ...xs[i] });
  }
  return out;
}
